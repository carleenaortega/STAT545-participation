...
#tidy is relative to what data we want but this is relatively tidy since each value is a column
...
lotr_wide %>% 
  pivot_longer(cols = c(-Film, -Gender), 
               ...
               values_to = "Words")
```
#- means "not" so that column will not be included
...
(guest_long <- guest %>% 
  pivot_longer(cols      = c(-party,-name), 
               names_to  = c(".value","event"), #separates the meal and event thats why result has "event" and "meal" as separate columns
               names_sep="_"))
```
...
```{r}
guest_long %>%  #use the newer table made (why?)
  group_by(party) %>% 
