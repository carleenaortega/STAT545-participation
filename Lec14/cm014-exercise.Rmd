...
before starting with model fitting, make sure you look at your data set.

Various types of plots can have the same summary statistics, which can be misleading so always look at your points..
...
```
I creates a new variable on the fly. This shifts the y axis from 0 to 1952
...
unclass(my_lm)
```
unclass takes a while to run because it has many functions.
...
predict(my_lm, newdata=gapminder_France2) %>% 
  head()
```
by adding the year seq we get the predicted data for the years 2000 to 2005.
```{r}
plot(my_lm)
```
...
ggplot(gapminder, aes(gdpPercap, lifeExp)) +
    ...
    scale_x_log10()
```
se=F removes the confidence interval of the smooth curve.
