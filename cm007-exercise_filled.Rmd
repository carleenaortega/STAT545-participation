...
2. Calculate the growth in population since the first year on record (1952) _for each country_ by rearranging the following lines, and filling in the `FILL_THIS_IN`. Here's another convenience function for you: `dplyr::first()`. 
...
gapminder %>% 
  group_by(country) %>% <!---so we can have the data sorted by country--->
  arrange(year) %>% <!---to have the numbers sorted by country and then year--->
  mutate(inc_life_exp = difference(lifeExp)) %>% <!---creates a new row that subtracts the life expectancies--->
  drop_na() %>%  <!---to remove NA values--->
  ungroup() %>% 
  group_by(continent) %>% 
  filter(inc_life_exp == min(inc_life_exp)) %>% 
  arrange(inc_life_exp) %>% <!---to arrange by continent--->
  knitr::kable()<!---improve display--->
