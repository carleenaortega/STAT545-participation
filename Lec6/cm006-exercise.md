...
```r
# compare
select(gapminder, continent = cont)
```

```
## Error in .f(.x[[i]], ...): object 'cont' not found
```
...
```r
gapminder %>%
  mutate(if_else(lifeExp=68.8,
    TRUE))
```

```
## Error in if_else(lifeExp = 68.8, TRUE): unused argument (lifeExp = 68.8)
```
